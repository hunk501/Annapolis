...
class Search extends Controller {
    ...
    public function comelec($application_id) {
    	...
    	if(!empty($records->toArray())) {    		    		
			...
			$keepGoing = true;
			$borrower = $this->parseName($borrower_name);
			if(!empty($borrower['lastname']) && !empty($borrower['middlename'])) {
				foreach($borrower['middlename'] as $bkey => $bMname) {
					if($keepGoing == false) {
						break;
					}

					// Search in Comelec for possible Records
					$comelec = MdlComelec::where('voters_name', 'LIKE', $borrower['lastname'].'%')
								->where('voters_name', 'LIKE', '%'.$bMname.'%')->get();
					
					if(!empty($comelec->toArray())) {
						foreach($comelec as $ckey => $cRecord) {
							$comName = $this->parseName($cRecord['voters_name']);
							
							if(!empty($comName['lastname']) && !empty($comName['middlename'])) {
								foreach($comName['middlename'] as $com_key => $com_mname) {

									// check if match
									if(strtolower($borrower['lastname']) == strtolower($comName['lastname']) 
										&& strtolower($bMname) == strtolower($com_mname)) {
											
											// Found
											array_push($arrFoundComelec, $cRecord);
											$keepGoing = false;
											$total++;
									}
								}
			...
			$output['records'] = $arrFoundComelec;
			if($total <= 0) {
				$output['error_msg'] = "No match record in comelec.!";	
			}
    	} else {
			$output['error_msg'] = "Borrower's Name was not found.!";
		}
	...
	private function parseName($strName) {
		// SEVILLA, RAQUEL DELOS SANTOS JR.
		$lname = null;
		$firstname = null;
		$arrMname = array();

		// Get Lastname
		$spltName = explode(",", $strName);
		if(count($spltName) >= 2) {
			$lname = $spltName[0]; // Lastname

			// Get Middlename
			$spltMname = explode(" ", trim($spltName[1]));
			$arrMnameTmp = array();			
			if(count($spltMname) >= 2) {
				foreach($spltMname as $key => $value) {
					if(strpos($value, ".") == false) { // Remove JR. SR.										
						array_push($arrMnameTmp, trim($value));
					}
				}
			}

			$cnt = count($arrMnameTmp);
			if($cnt >= 2) {
				if($cnt <= 2) {
					array_push($arrMname, $arrMnameTmp[1]);
					$firstname = $arrMnameTmp[0];
				} else {
					$tmp = array();
					$arrMnameTmp = array_reverse($arrMnameTmp);
					$x = 1;
					foreach($arrMnameTmp as $k => $v) {
						if($x <= 2) {
							array_push($tmp, trim($v));
						} else {
							$firstname .= trim($v);
						}
						$x++;
					...
					if(count($tmp) >= 2) {
						// Lists possible Middlename
						$mname1 = trim($tmp[0]) ." ". trim($tmp[1]); // 1st. DELOS SANTOS
						$mname2 = trim($tmp[1]);                     // 2nd. SANTOS
						$mname3 = trim($tmp[0]);                     // 3rd. DELOS
						array_push($arrMname, $mname1);
						array_push($arrMname, $mname2);
						//array_push($arrMname, $mname3);
					}
				}
			}
		}
